  ...
  BaseDriver.prototype.transformResult = function (result, action, options) {
    ...
    var shouldTransform =
      ...
      !(
        ...
        ) // don't transform streams
      );
    // transform result if needed
    if (shouldTransform) {
      var options_1 =
        action.responseClassTransformOptions ||
    ...
    if (error instanceof Error) {
      ...
      if (this.errorOverridingMap)
        Object.keys(this.errorOverridingMap)
          ...
          .forEach(function (key) {
            return (processedError = _this.merge(
              processedError,
              _this.errorOverridingMap[key]
            ));
          });
      return Object.keys(processedError).length > 0
        ? processedError
        : undefined;
    }
    return error;
